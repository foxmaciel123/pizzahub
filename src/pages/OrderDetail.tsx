...
import { FileText, ShieldCheck, Sparkles, CheckCircle2 } from 'lucide-react'

export const OrderDetail: React.FC = () => {
  const { id } = useParams()
  const displayId = id || 'demo-001'
      ...
      <div className="p-4 sm:p-8 max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-800/80 pb-5">
          <div className="space-y-1">
            <div className="flex items-center gap-2 text-xs text-slate-400">
              ...
              <span>/</span>
              <span className="text-orange-400 font-medium">Detalhes do Pedido</span>
            </div>
            <h1 className="text-2xl font-bold text-white tracking-tight flex items-center gap-2.5">
              <FileText className="w-6 h-6 text-orange-500 shrink-0" />
              <span>Detalhes do Pedido #{displayId}</span>
            </h1>
            <p className="text-xs text-slate-400 max-w-2xl leading-relaxed">
              Visão completa dos itens, opções de borda/adicionais, endereço de entrega e dados do cliente.
            </p>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="px-2.5 py-1 rounded-lg text-[11px] font-semibold border text-sky-400 border-sky-500/30 bg-sky-500/10">
              Dono / Gerente • Atendente • Cozinha
            </span>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3.5">
            <div key="0" className="p-4 rounded-2xl bg-slate-900/60 border border-slate-800/80 hover:border-slate-700 transition-all space-y-2">
              <div className="text-xs font-bold text-white flex items-center gap-1.5">
                ...
                <span>Composição de Sabores</span>
              </div>
              <p className="text-[11px] text-slate-400 leading-relaxed">Discriminação de meio-a-meio, tipo de massa, borda recheada e observações da cozinha.
              </p>
            </div>
            <div key="1" className="p-4 rounded-2xl bg-slate-900/60 border border-slate-800/80 hover:border-slate-700 transition-all space-y-2">
              <div className="text-xs font-bold text-white flex items-center gap-1.5">
                ...
                <span>Status do PDV & Entrega</span>
              </div>
              <p className="text-[11px] text-slate-400 leading-relaxed">Rastreamento da emissão no PDV e sincronização de status de volta ao canal de origem.
              </p>
            </div>
            <div key="2" className="p-4 rounded-2xl bg-slate-900/60 border border-slate-800/80 hover:border-slate-700 transition-all space-y-2">
              ...
              <p className="text-[11px] text-slate-400 leading-relaxed">Últimos pedidos realizados pelo cliente, canal preferido e endereço recorrente.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 rounded-2xl bg-slate-900/40 border border-slate-800/60 flex items-center justify-between text-xs text-slate-400">
          <div className="flex items-center gap-2">
            ...
            <span>Nível de Acesso: <strong>Dono / Gerente, Atendente, Cozinha</strong></span>
          </div>
